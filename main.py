...
def generate_meal_plan_pipeline(user_id):
    """
    Runs the entire pipeline to generate a meal plan for a given user.
    """
    ...
    try:
        ...
        for day in days_to_process:
            ...
            for meal_key, meal_val in day_content.items():
                ...
                if dish_name:
                    ...
                    if img_bytes:
                        file_id = save_image_to_gridfs(
                            img_bytes, f"{day}_{dish_name.replace(' ', '_')}.png"
                        )
        ...
        existing = meal_plan_collection.find_one({"user_id": user_id})
        if existing:
            meal_plan_collection.replace_one({"_id": existing["_id"]}, record)
            print(f"Updated meal plan with images for user {user_id} in MongoDB.")
        else:
            meal_plan_collection.insert_one(record)
            print(f"Inserted meal plan with images for user {user_id} in MongoDB.")
    ...
    for day, meals in meal_plan.items():
        ...
        for meal_name, meal_details in meals.items():
            ...
            try:
                ...
                print(
                    f"⚠️ Could not parse the recipe for {dish_name}. Response was: {raw_content}"
                )
...
def generate_shopping_list_pipeline(meal_plan_doc):
    """
    Generates a shopping list for the given meal plan.
    """
    if not meal_plan_doc:
        print("❌ No meal plan found in the database.")
        return
    ...
    if shopping_list:
        print(f"\n💾 Saving shopping list to 'IngredientsCol' collection...")
        try:
            ingredient_doc = {
                "user_id": user_id,
            ...
            result = ingredient_collection.insert_one(ingredient_doc)
            print(
                f"✅ Successfully saved ingredients with document ID: {result.inserted_id}"
            )
        except Exception as e:
    ...
    for document in all_documents_cursor:
        ...
        if "shopping_list" in document and isinstance(document["shopping_list"], dict):
            ...
            try:
                ...
                print(
                    f"❌ An unexpected error occurred while updating doc {doc_id}: {e}\n"
                )
        else:
            print(
                f"⚠️ Document {doc_id} does not have a 'shopping_list' field. Skipping.\n"
            )
