...
st.markdown(
    ...
    <style>
        ...
        .dish-name { font-weight: 800; font-size: 1.6em; margin-top: 8px; margin-bottom: 6px; text-align: center; color: #FFFFFF; }
...
# --- Create Tabs ---
tabs = ["Step 1: Profile", "Step 2: Meal Plan", "Step 3: Recipes", "Step 4: Shopping List", "Step 5: Get on WhatsApp"]
...
with tab4:
    ...
    if not phone_input or not st.session_state.get('recipes_generated'): st.warning("Please generate Recipes in Tab 3 to create a shopping list.")
    else:
        if st.button("🛒 Generate Shopping List & Prices"):
            with st.spinner("🧠 Analyzing recipes and forecasting prices..."):
                try:
                    user = user_collection.find_one({"phone": phone_input}); user_id = user["_id"]
                    meal_plan_doc = meal_plan_collection.find_one({"user_id": user_id}); meal_plan_data = meal_plan_doc.get("meal_plan", {})
                    response = shopping_agent.run(json.dumps(clean_mongo_doc(meal_plan_data), indent=2))
                    match = re.search(r'(\{.*\})', response.content.strip(), re.DOTALL); shopping_list = json.loads(match.group(1))
                    
                    st.write("Forecasting prices for your list...")
                    price_response = price_agent.run(json.dumps(shopping_list))
                    price_match = re.search(r'(\{.*\})', price_response.content.strip(), re.DOTALL); pricing_details = json.loads(price_match.group(1))

                    ingredient_doc = { "user_id": user_id, "source_meal_plan_id": meal_plan_doc["_id"], "shopping_list": shopping_list, "pricing_details": pricing_details, "created_at": datetime.now(timezone.utc) }
                    ingredient_collection.update_one({"source_meal_plan_id": meal_plan_doc["_id"]}, {"$set": ingredient_doc}, upsert=True)
                    st.success("✅ Shopping list and prices generated!"); st.session_state['shopping_list_generated'] = True
                except Exception as e: st.error(f"An error occurred: {e}")
        ...
        if st.session_state.get('shopping_list_generated'):
            ...
            if shopping_list_doc:
                ...
                for category, details in pricing_details.items():
                    ...
                    # De-duplicate items before processing
                    unique_items = []; seen_names = set()
                    for item in items_with_prices:
                        name = item.get("name")
                        if name and name not in seen_names:
                            unique_items.append(item); seen_names.add(name)
                ...
                for category, data in all_categories_data.items():
                    ...
                    edited_df = st.data_editor(df, key=f"df_{category}", hide_index=True, use_container_width=True, disabled=["Item", "Price (₹)"])
